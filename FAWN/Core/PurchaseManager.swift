...
//  FAWN
//
//  Created by Алексей Петров on 22/08/2019.
...
class PurchaseManager: NSObject {
    ...
    public var showUponLogin: Bool {
        return AppManager.shared.showPaygate
    }
    ...
    func getStoreContry(_ completion:@escaping(_ storeCountry: String) -> Void) {
        let request: PurchaseRequset = PurchaseRequset()
        RequestManager.shared.requset(request) { (responce) in
             if responce != nil {
                let paygateData: PaymentResponse = responce as! PaymentResponse
                if paygateData.httpCode == 200 {
                    SwiftyStoreKit.retrieveProductsInfo(self.getProductIDs(from: paygateData.data!)) { (result) in
                        if let product: SKProduct = result.retrievedProducts.first {
                            completion(product.priceLocale.currencyCode!)
                        } else {
                            completion("")
                        }
                    }
